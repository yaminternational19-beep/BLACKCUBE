...
const Button = ({
  ...
  const commonClasses = cn(baseClasses, variants[variant], sizes[size], className);
  const content = isLoading ? <div className="flex items-center space-x-2">
      <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
      <span>Loading...</span>
    </div> : children;
  if (href) {
    return <Link to={href} className={commonClasses} {...props}>
        {content}
      </Link>;
  }
  return <button className={commonClasses} disabled={disabled || isLoading} {...props}>
      {content}
    </button>;
};
